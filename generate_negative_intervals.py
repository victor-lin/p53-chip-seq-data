...
def output_df_with_negatives(merged_bed_macs, chrom_sizes):
    ...
    df_negative1['high'] = df_negative1.apply(lambda row: min(row['start'],
                                                              10000), axis=1)
    df_negative1['low'] = df_negative1.apply(lambda row: min(max(row['high'] - 9000, 0),
                                                             1000), axis=1)
    df_negative1['rand_shift'] = df_negative1.apply(lambda row: np.random.randint(row['low'], row['high']), axis=1)
    ...
    df_negative2['high'] = df_negative2.apply(lambda row: min(chrom_sizes_dict[row['chr']] - row['end'],
                                                              10000), axis=1)
    df_negative2['low'] = df_negative2.apply(lambda row: min(max(row['high'] - 9000, 0),
                                                             1000), axis=1)
    df_negative2['rand_shift'] = df_negative2.apply(lambda row: np.random.randint(row['low'], row['high']), axis=1)
    df_negative2['start'] = df_peaks['start'] + df_negative2['rand_shift']
    df_negative2['end'] = df_peaks['start'] + df_negative2['rand_shift'] + df_peaks['length']
    df_negative2.drop(['high', 'low', 'rand_shift'], axis=1, inplace=True)

    # TODO: drop rows if overlap or too close to original
