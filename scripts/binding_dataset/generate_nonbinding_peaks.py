...
def generate_nonbinding_intervals(bed_df, genome_seq_records,
    ...
    rep_cutoff_type : str (min/max)
        repeat proportion cutoff type for binding intervals to be included
    rep_cutoff : float
        cutoff value for rep_cutoff_type
    min_dist : int
...
def generate_random_intervals(chrom, lower, upper, length, genome_seq_records,
    ...
    for interval in possible_intervals_copy:
        ...
        if rep_cutoff_type == 'max' and rep_prop > rep_cutoff:
            possible_intervals.remove(interval)
        if rep_cutoff_type == 'min' and rep_prop < rep_cutoff:
            possible_intervals.remove(interval)
...
if __name__ == "__main__":
    ...
    generate_nonbinding_intervals(df_binding, genome_seq_records,
                                  args.rep_cutoff_type, args.rep_cutoff,
